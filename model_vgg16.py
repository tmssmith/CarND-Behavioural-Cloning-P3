import csv
import tensorflow as tf
import cv2
import numpy as np
import sys
from keras.models import Sequential, Model
from keras.layers import Input, Flatten, Dense, Lambda, Conv2D, MaxPooling2D, Cropping2D
from keras.preprocessing.image import ImageDataGenerator
from keras.applications.inception_resnet_v2 import InceptionResNetV2
from keras.applications.inception_v3 import InceptionV3
from keras.applications.vgg16 import VGG16, preprocess_input
from sklearn.model_selection import train_test_split
from sklearn.utils import shuffle

## Data augmentation - flip images

log = []
with open('./data/driving_log.csv') as csvfile:
    reader = csv.reader(csvfile)
    next(reader)  # Skip headers
    for line in reader:
        log.append(line)

# training, validation = train_test_split(samples, test_size=0.2)

# def generator(samples, batch_size=32, preprocess_flag=True):
#     num_samples = len(samples)    
#     steer_correction = [0, 0.5, -0.5]
#     while 1: # Loop forever so the generator never terminates
#         shuffle(samples)
#         for offset in range(0, num_samples, batch_size):
#             batch_samples = samples[offset:offset+batch_size]
#             frames = []
#             angles = []
            
#             for batch_sample in batch_samples:
#                 for cam in range(3):
#                     frame_path = './data/' + batch_sample[cam].strip()
#                     frame = cv2.cvtColor(cv2.imread(frame_path), cv2.COLOR_BGR2RGB)
#                     frame_flip = np.fliplr(frame)
#                     frames.extend((frame, frame_flip))
#                     angle = float(batch_sample[3]) + steer_correction[cam]
#                     angle_flip = -angle
#                     angles.extend((angle, angle_flip))

#             X = np.array(frames)
#             y = np.array(angles)
            
#             if preprocess_flag == True:
#                 train_datagen = ImageDataGenerator(preprocessing_function=preprocess_input)
#                 valid_datagen = ImageDataGenerator(preprocessing_function=preprocess_input)
#             else:
#                 train_datagen = ImageDataGenerator()
#                 valid_datagen = ImageDataGenerator()    
    
#             yield (train_datagen, valid_datagen)
            
            
frames = []
angles = []
steering_correction = 0.5

for line in log:
    frame_path = './data/' + line[0].strip()
    frame = cv2.cvtColor(cv2.imread(frame_path), cv2.COLOR_BGR2RGB)
    frame_flip = np.fliplr(frame)
    frames.extend((frame, frame_flip))
    angle = float(line[3])
    angles.extend((angle, -angle))

X = np.array(frames)
y = np.array(angles)

X_train, X_valid, y_train, y_valid = train_test_split(X,y, test_size=0.2)

freeze_flag = True  # `True` to freeze layers, `False` for full training
weights_flag = 'imagenet' # 'imagenet' or None
preprocess_flag = True

# inception = InceptionV3(weights=weights_flag, include_top=False, input_shape=(299, 299, 3), pooling='avg')
vgg = VGG16(weights=weights_flag, include_top=False, input_shape=(224,224,3), pooling='avg')
# resnet = InceptionResNetV2(weights=weights_flag, include_top=False, input_shape=(299, 299, 3), pooling='avg')

if freeze_flag == True:
    for layer in vgg.layers:
        layer.trainable = False

cam_input = Input(shape=(160,320,3))
cropped = Cropping2D(cropping=((70,25),(0,0)), input_shape=(160,320,3))(cam_input)
resized = Lambda(lambda image: tf.image.resize_images( 
    image, (224, 224)))(cropped)
inp = vgg(resized)
x = Dense(256, activation='relu')(inp)
predictions = Dense(1)(x)

model = Model(inputs=cam_input, outputs=predictions)
model.compile(loss='mse', optimizer='adam')

batch_size = 32

# train_gen = generator(training, batch_size)
# valid_gen = generator(validation, batch_size)

X_train, y_train = shuffle(X_train, y_train)
X_valid, y_valid = shuffle(X_valid, y_valid)

if preprocess_flag == True:
    train_datagen = ImageDataGenerator(preprocessing_function=preprocess_input)
    valid_datagen = ImageDataGenerator(preprocessing_function=preprocess_input)
else:
    train_datagen = ImageDataGenerator()
    valid_datagen = ImageDataGenerator()

epochs = 3
    
# model.fit(train_gen(training, batch_size=batch_size), 
#                     steps_per_epoch=len(training)*6/batch_size, epochs=epochs, 
#                     validation_data=valid_gen(validation, batch_size=batch_size),
#                     validation_steps=len(validation)*6/batch_size)

model.fit_generator(train_datagen.flow(X_train, y_train, batch_size=batch_size), 
                    steps_per_epoch=len(X_train)/batch_size, epochs=epochs, 
                    validation_data=valid_datagen.flow(X_valid, y_valid, batch_size=batch_size),
                    validation_steps=len(X_valid)/batch_size)


# model = Sequential()
# model.add(Cropping2D(cropping=((65,20),(0,0)), input_shape=(160,320,3)))
# model.add(Lambda(lambda x: x / 255.0 - 0.5))
# model.add(Conv2D(6, kernel_size=(3,3), activation='relu'))
# model.add(MaxPooling2D())
# model.add(Conv2D(16, kernel_size=(3,3), activation='relu'))
# model.add(MaxPooling2D())
# model.add(Flatten())
# model.add(Dense(120))
# model.add(Dense(84))
# model.add(Dense(1))

# model.compile(loss='mse', optimizer='adam')

# model.fit(X, y, validation_split=0.2, shuffle=True, epochs=4)

model.save('model.h5')


    